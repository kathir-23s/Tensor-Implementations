...
Tensor _reduce_dispatcher(const Tensor& input, const std::vector<int64_t>& axes, bool keepdim) {
    if (input.device().device != Device::CPU) {
        // NOTE: Device is now OwnTensor::Device, but since we are inside OwnTensor namespace, 
        // we can refer to it directly as Device::CPU
        throw std::runtime_error("Reduction only supported on CPU devices for now.");
    }
