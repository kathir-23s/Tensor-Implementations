...
__global__ void unary_half_kernel_gpu(const half* in, half* out, size_t size) {
    ...
    if (idx < size) {
        ...
        f32_val = expf(f32_val);                 // compute exp
...
Tensor exp_out_gpu_wrap(const Tensor& input_tensor){
    ...
    unary_half_kernel_gpu<<<blocks, threads>>>(in, out, size);
    ...
    unary_bfloat16_kernel_gpu<<<blocks, threads>>>(in, out, size);
    cudaDeviceSynchronize();
    } else {
...
void exp_in_gpu_wrap(Tensor& input_tensor){
    try {
        std::cout << "Inplace CUDA exp() called!\n";
        ...
        if (dtype == Dtype::Float32) {
            std::cout << "Running inplace exp for Float32 on GPU\n";
            const float* in_cptr = input_tensor.data<float>();
            float* in_ptr = input_tensor.data<float>();
            unary_kernel_gpu<float, float, expf><<<blocks, threads>>>(in_cptr, in_ptr, size);
        }
        else if (dtype == Dtype::Float64) {
            std::cout << "Running inplace exp for Float64 on GPU\n";
            const double* in_cptr = input_tensor.data<double>();
            double* in_ptr = input_tensor.data<double>();
            unary_kernel_gpu<double, double, exp><<<blocks, threads>>>(in_cptr, in_ptr, size);
        }
        else if (dtype == Dtype::Float16) {
            std::cout << "Running inplace exp for Float16 on GPU\n";
            half* in_cptr = input_tensor.data<half>();
            half* in_ptr = input_tensor.data<half>();
            unary_half_kernel_gpu<<<blocks, threads>>>(in_cptr, in_ptr, size);
        }
        else if (dtype == Dtype::Bfloat16) {
            std::cout << "Running inplace exp for Bfloat16 on GPU\n";
            __nv_bfloat16* in_cptr = input_tensor.data<__nv_bfloat16>();
            __nv_bfloat16* in_ptr = input_tensor.data<__nv_bfloat16>();
            unary_bfloat16_kernel_gpu<<<blocks, threads>>>(in_cptr, in_ptr, size);
        }
