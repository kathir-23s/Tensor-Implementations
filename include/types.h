...
struct bfloat16_t {
    ...
    explicit bfloat16_t(float val) {
        raw_bits = float_to_bfloat16(val);
    }

    bfloat16_t(const bfloat16_t& other) : raw_bits(other.raw_bits) {}

    template <typename U, typename = std::enable_if_t<std::is_arithmetic_v<U> && !std::is_same_v<std::decay_t<U>, float>>>
    explicit bfloat16_t(U val) {
        raw_bits = float_to_bfloat16(static_cast<float>(val));
    }

    // ---- Conversion ----
    operator float() const {
        return bfloat16_to_float(raw_bits);
    }
...
struct float16_t {
    ...
    explicit float16_t(U val) {
        raw_bits = float_to_float16(static_cast<float>(val));
    }

    // ---- Conversion ----
    operator float() const {
        return float16_to_float(raw_bits);
    }
...
// ---- BFloat16 Limits ----
template <>
struct numeric_limits<bfloat16_t> : public numeric_limits_fp16_helper<bfloat16_t> {};

template <>
inline bfloat16_t numeric_limits_fp16_helper<bfloat16_t>::lowest() noexcept {
    return bfloat16_t(-3.38953e38f);
}

template <>
inline bfloat16_t numeric_limits_fp16_helper<bfloat16_t>::max() noexcept {
    return bfloat16_t(3.38953e38f);
}

// ---- Float16 Limits ----
template <>
struct numeric_limits<float16_t> : public numeric_limits_fp16_helper<float16_t> {};

template <>
inline float16_t numeric_limits_fp16_helper<float16_t>::lowest() noexcept {
    return float16_t(-65504.0f);
}

template <>
inline float16_t numeric_limits_fp16_helper<float16_t>::max() noexcept {
    return float16_t(65504.0f);
}
//
} // namespace std
 ...
 * bfloat16_t bf16_val(3.14f);
 * float16_t fp16_val(2.71f);
 ...
 * auto res = compute(bf16_val, bfloat16_t(5.0f));
 * 
 * // 4. Type traits
 * static_assert(is_half_float_v<bfloat16_t>);
 * static_assert(is_any_float_v<float>);
 * 
 * // 5. Numeric limits
 * auto max_bf16 = std::numeric_limits<bfloat16_t>::max();
 * auto inf_fp16 = std::numeric_limits<float16_t>::infinity();
 * 
 * ==================================================================================
 */