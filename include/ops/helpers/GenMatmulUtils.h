...
namespace OwnTensor
{
    // Renamed from the first implementation
    void cpu_matmul_general(const Tensor& A, const Tensor& B, Tensor& output)
    {
        dispatch_by_dtype(A.dtype(), [&](auto dummy)
        {
           ...
           [[maybe_unused]] size_t out_ndim = out_shape.size();
    ...
    // Renamed from the second implementation (optimized single loop)
    void cpu_matmul_optimized(const Tensor& A, const Tensor& B, Tensor& output)
    {
        dispatch_by_dtype(A.dtype(), [&](auto dummy)
            {
                using T = decltype(dummy);
                ...
                for (size_t i = 0; i < m * p; ++i)
                {
                    ...
                    size_t a_row_offset = row * n;

                    for (size_t k = 0; k < n; ++k)
                    {
                        // Optimized indexing assuming contiguous memory
                        sum += a_ptr[a_row_offset + k] * b_ptr[k * p + col];
                    }
                    out_ptr[i] = sum;
    ...
    // Wrapper to maintain compatibility if needed, but for benchmark we call specific ones
    void cpu_matmul(const Tensor& A, const Tensor& B, Tensor& output)
    {
        // Default to general for safety in normal code
        cpu_matmul_general(A, B, output);
    }
